...
using Octoplex.Impostos.WebAPI.Extensions;
using MediatR;
...
builder.Services.AddMediatR(typeof(ImpostoCommand));
builder.Services.AddMediatR(typeof(ImpostoCalcularCommand));
...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "CK.Impostos.WebAPI", Version = "v1" });
...
builder.Services.UseDbContext<ImpostoDbContext>(builder.Configuration);

//Interface(Domain),  Implementacao (Infra.Data)
//Registrando Scopo (<IEmpresaRepository, EmpresaRepository>)
builder.Services.AddScoped<IImpostosRepository, ImpostosRepository>();
builder.Services.AddTransient<IImpostosRepository, ImpostosRepository>();
...
if (app.Environment.IsDevelopment())
{
    ...
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "CK.Impostos.WebAPI v1");
    });
}
